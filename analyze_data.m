...
%             rh_all = rh_all - repmat([0.8 0.3], [size(rh_all,1) 1 size(rh_all,3)]);
            ...
            for k = 1:size(output,3)
                MSE(k) = mean((rh_all(:,1,k)-t_all(:,1,k)).^2 + (rh_all(:,2,k)-t_all(:,2,k)).^2);
            end
...
%             create data structures to store all data
            ...
            data.(subj_name{i}).(block_name{j}).x_x_all = fourier(Rhand_all.x_pos, target_all.x_pos, length(freqs_x));
            data.(subj_name{i}).(block_name{j}).y_y_all = fourier(Rhand_all.y_pos, target_all.y_pos, length(freqs_y));
            data.(subj_name{i}).(block_name{j}).x_y_all = fourier(Rhand_all.y_pos, target_all.x_pos, length(freqs_x));
            data.(subj_name{i}).(block_name{j}).y_x_all = fourier(Rhand_all.x_pos, target_all.y_pos, length(freqs_y));
...
%             cohyy2 = mscohere(target.y_pos,cursor_all.y_pos,blackmanharris(round(N/5)),[],freqs_y,fs);
            ...
            total = 0;
            for k = 1:size(Rhand_all.x_pos,2)-1
                total = total + k;
            end

            FLAG = 1;
            k1 = 1;
            k2 = size(Rhand_all.x_pos,2);
            k3 = 1;
            cohx = NaN(total,length(freqs_x));
            cohy = NaN(total,length(freqs_y));
            while FLAG
                cohx(k3,:) = mscohere(Rhand_all.x_pos(:,k1),Rhand_all.x_pos(:,k2),[],[],freqs_x,fs);
                cohy(k3,:) = mscohere(Rhand_all.y_pos(:,k1),Rhand_all.y_pos(:,k2),[],[],freqs_y,fs);
                k1 = k1 + 1;
                if k1 == k2
                    k1 = 1;
                    k2 = k2 - 1;
