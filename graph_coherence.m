...
for p = 1:length(groups)
    for i = 1:Nsubj
        for j = 1:Nblock
            targetX = data.(groups{p}){i}.(block_name{j}).target.x_pos_all; % target position data in all trials
            targetY = data.(groups{p}){i}.(block_name{j}).target.y_pos_all;
            outX = data.(groups{p}){i}.(block_name{j}).(output).x_pos_all; % hand position data in all trials
            outY = data.(groups{p}){i}.(block_name{j}).(output).y_pos_all;
            N = size(targetX,1);
            
            % calculate coherence
            for k = 1:Ntrials
                coh = mscohere([outX(:,k) outY(:,k)],[targetX(:,k) targetY(:,k)],blackmanharris(round(N/5)),[],sorted_freqs,130.004,'mimo')';
                cohxx(k,:) = coh(1,:); % store target-hand x coherence
                cohyy(k,:) = coh(2,:); % store target-hand y coherence
            end
            SR.x_all(j,:,i,p) = mean(cohxx(:,1:2:end)); % average across trials
            SR.y_all(j,:,i,p) = mean(cohyy(:,2:2:end));
        end
    end
end

SR.x = squeeze(mean(SR.x_all,3)); % average coherence across subjects
SR.y = squeeze(mean(SR.y_all,3));
SR.xSE = squeeze(std(SR.x_all,[],3)/sqrt(Nsubj)); % standard error of coherence across subjects
