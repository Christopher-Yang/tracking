...
for l = 1:Ngroup
    for i = 1:Nsubj
        for j = 1:Nblock
            ...
            % hand position
            hand = [mean(dat.Rhand.x_pos,2) mean(dat.Rhand.y_pos,2)]';
            % target position
            target = [mean(dat.target.x_pos,2) mean(dat.target.y_pos,2)]';
            for k = 1:length(delay) % compute MSE for given delay
                ...
                transformMat(:,:,k) = [params_opt(1:2); params_opt(3:4)]; 
...
% in both axes
for i = 1:4 
    x(:,:,i,1) = cov(squeeze(transformMat_vmr(1,1,gblocks(i),:)),...
        squeeze(transformMat_vmr(2,1,gblocks(i),:)));
    y(:,:,i,1) = cov(squeeze(transformMat_vmr(1,2,gblocks(i),:)),...
        squeeze(transformMat_vmr(2,2,gblocks(i),:)));
    x(:,:,i,2) = cov(squeeze(transformMat_mr(1,1,gblocks(i),:)),...
        squeeze(transformMat_mr(2,1,gblocks(i),:)));
    y(:,:,i,2) = cov(squeeze(transformMat_mr(1,2,gblocks(i),:)),...
        squeeze(transformMat_mr(2,2,gblocks(i),:)));
end
...
for i = 1:4
    ...
    a = error_ellipse(x(:,:,i,1),mat1(:,1,gblocks(i)),'color',col1,...
        'conf',0.95); % draw confidence ellipses 
    b = error_ellipse(y(:,:,i,1),mat1(:,2,gblocks(i)),'color',col2,...
        'conf',0.95);
    patch(a.XData,a.YData,col1,'FaceAlpha',0.2) % shade confidence ellipses
    patch(b.XData,b.YData,col2,'FaceAlpha',0.2)
    plot([0 mat1(1,1,gblocks(i))],[0 mat1(2,1,gblocks(i))],'Color',col1,...
        'LineWidth',1.5) % plot vectors
    plot([0 mat1(1,2,gblocks(i))],[0 mat1(2,2,gblocks(i))],'Color',col2,...
        'LineWidth',1.5)
    ...
    plot([0 1],[0 0],'k') % unit x vector
    plot([0 0],[0 1],'k') % unit y vector
    a = error_ellipse(x(:,:,i,1),mat2(:,1,gblocks(i)),'color',col1...
        ,'conf',0.95); % draw confidence ellipses
    b = error_ellipse(y(:,:,i,1),mat2(:,2,gblocks(i)),'color',col2...
        ,'conf',0.95);
    patch(a.XData,a.YData,col1,'FaceAlpha',0.2) % shade confidence ellipses
    patch(b.XData,b.YData,col2,'FaceAlpha',0.2)
    plot([0 mat2(1,1,gblocks(i))],[0 mat2(2,1,gblocks(i))],'Color',col1,...
        'LineWidth',1.5) % plot vectors
    plot([0 mat2(1,2,gblocks(i))],[0 mat2(2,2,gblocks(i))],'Color',col2,...
        'LineWidth',1.5)
    axis([-0.9 1.2 -0.9 1.2])
    axis square
    if i == 1
...
vmr = cat(3,squeeze(-transformMat_vmr(1,2,:,:)),squeeze...
    (transformMat_vmr(2,1,:,:)));
mr = cat(3,squeeze(transformMat_mr(1,2,:,:)),squeeze...
    (transformMat_mr(2,1,:,:)));
vmr = mean(vmr,3);
mr = mean(mr,3);
...
scaleOrthMu = mean(scaleOrth,2); % mean of MR group's orthogonal scaling
