s = zpk('s');
G = 1/s;
[err.x, model.x] = predictionError(data, G, 'x', 1);
[err.y, model.y] = predictionError(data, G, 'y', 1);
...
loglog([1 1],[1 1],'b')
loglog(freq.x,squeeze(err.x.fb_ff),'Color',[1 0 0 0.4])
loglog(freq.x,squeeze(err.x.fb),'Color',[0 0 1 0.4])
loglog(freq.x,mean(err.x.fb_ff,2),'Color','r','LineWidth',2)
loglog(freq.x,mean(err.x.fb,2),'Color','b','LineWidth',2)
title('X axis')
xlabel('Frequency (Hz)')
...
subplot(1,2,2)
loglog(freq.y,squeeze(err.y.fb_ff),'Color',[1 0 0 0.4]); hold on
loglog(freq.y,squeeze(err.y.fb),'Color',[0 0 1 0.4])
loglog(freq.y,mean(err.y.fb_ff,2),'Color','r','LineWidth',2)
loglog(freq.y,mean(err.y.fb,2),'Color','b','LineWidth',2)
title('Y axis')
xlabel('Frequency (Hz)')
ylim([30 6000])
box off
...
%         F = reshape(permute(model.(fields1{i}).(fields3{k}),[1 3 2]),[Nfreq Nsubj*Ntrials]);
        ...
        B = model.(fields1{i}).(fields2{k});
        F = model.(fields1{i}).(fields3{k});
...
%         end
        ...
        plot(freq.(fields1{i}),mean(B,2),'b','LineWidth',3)
        plot(freq.(fields1{i}),mean(F,2),'r','LineWidth',3)
        plot(freq.(fields1{i}),B,'Color',[0 0 1 0.4])
...
%%
function [err, model] = predictionError(data, G, axis, block)
s = data{1}.sineParams;
...
for i = 1:Nsubj
    ...
    Hur(:,i) = a.(pAxis{1}){4*(block-1)+1}.ratio;
    Hud(:,i) = a.(pAxis{2}){4*(block-1)+2}.ratio;
    
    Hur2(:,i) = reshape(permute([a.(pAxis{1}){4*(block-1)+3}.ratio a.(pAxis{1}){4*(block-1)+4}.ratio],[2 1]),[Nfreq 1]);
    Hud2(:,i) = reshape(permute([a.(pAxis{2}){4*(block-1)+4}.ratio a.(pAxis{2}){4*(block-1)+3}.ratio],[2 1]),[Nfreq 1]);
    
    idx1 = [a.index.(fAxis{1}){4*(block-1)+3}; a.index.(fAxis{1}){4*(block-1)+4}];
    idx2 = [a.index.(fAxis{2}){4*(block-1)+3}; a.index.(fAxis{2}){4*(block-1)+4}];
    
    r = [b.target.(fftAxis)(idx1(1,:),4*(block-1)+3) b.target.(fftAxis)(idx1(2,:),4*(block-1)+4)];
    d = [b.cursorInput.(fftAxis)(idx2(1,:),4*(block-1)+4) b.cursorInput.(fftAxis)(idx2(2,:),4*(block-1)+3)];
    R(:,i) = reshape(permute(r,[2 1]),[Nfreq 1]);
    D(:,i) = reshape(permute(d,[2 1]),[Nfreq 1]);
end
