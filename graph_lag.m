...
%         figure(o)
%         subplot(2,2,(i-1)*2 + 1)
%         semilogx(f_x,delay.x_x','-o','LineWidth',1.5)
%         set(gca,'FontSize',12,'TickDir','out','LineWidth',1)
%         title([group_names,' (X)'],'FontSize',15)
...
%         
%         subplot(2,2,(i-1)*2 + 2)
%         semilogx(f_y,delay.y_y','-o','LineWidth',1.5)
%         set(gca,'FontSize',12,'TickDir','out','LineWidth',1)
%         title([group_names,' (Y)'],'FontSize',15)
%         ylabel('Response Lag (ms)','FontSize',15)
%         xlabel('Frequency (Hz)','FontSize',15)
%         axis([0.09 2.3 300 1500]);
%         grid on
...
%             s = shadedErrorBar(f_x,delay.x_x(gblocks(j),:)',err.x_x(:,:,gblocks(j)),'lineProps','-o'); hold on;
...
%         subplot(2,2,i+2)
%         for j = 1:length(gblocks)
%             s = shadedErrorBar(f_y,delay.y_y(gblocks(j),:)',err.y_y(:,:,gblocks(j)),'lineProps','-o');
%             editErrorBar(s,col(j,:),3);
%         end
%         set(gca,'Xscale','log','FontSize',18,'Xgrid','on','GridAlpha',0.4,'MinorGridAlpha',0.5,'box','off','TickDir','out','LineWidth',1,'Ytick',300:300:1500)
%         if i == 1
...
%         legend(graph_name{gblocks},'Position',[0.34 0.8 0.1 0.1],'FontSize',18)
    ...
    s = shadedErrorBar(f_y,delay.y_y(gblocks(2),:)',err.y_y(:,:,gblocks(2)),'lineProps','-o');
    editErrorBar(s,col(2,:),2);
    s = shadedErrorBar(f_y,delay.y_y(gblocks(3),:)',err.y_y(:,:,gblocks(3)),'lineProps','-o');
    editErrorBar(s,col(3,:),2);
    s = shadedErrorBar(f_x,delay.x_x(gblocks(4),:)',err.x_x(:,:,gblocks(4)),'lineProps','-o');
    ...
    ylabel('Response Lag (ms)','FontSize',15)
    axis([0.09 2.3 200 600])

    subplot(2,1,2)
    s = shadedErrorBar(f_y,delay.y_y(gblocks(1),:)',err.y_y(:,:,gblocks(1)),'lineProps','-o'); hold on
    editErrorBar(s,col(1,:),2);
    s = shadedErrorBar(f_x,delay.x_x(gblocks(2),:)',err.x_x(:,:,gblocks(2)),'lineProps','-o');
    editErrorBar(s,col(2,:),2);
    s = shadedErrorBar(f_x,delay.x_x(gblocks(3),:)',err.x_x(:,:,gblocks(3)),'lineProps','-o');
    editErrorBar(s,col(3,:),2);
    s = shadedErrorBar(f_y,delay.y_y(gblocks(4),:)',err.y_y(:,:,gblocks(4)),'lineProps','-o');
    editErrorBar(s,col(4,:),2);
    set(gca,'Xscale','log','FontSize',12,'Xgrid','on','GridAlpha',0.4,'MinorGridAlpha',0.5,'box','off','TickDir','out','LineWidth',1,'Ytick',300:300:1500)
    ylabel('Response Lag (ms)','FontSize',15)
    xlabel('Frequency (Hz)','FontSize',15)
